...
void sched_new(void (*entrypoint)(void *aspace), void *aspace, int priority, int deadline) {
    ...
    if (!policy) {
        printf("Error: policy was not defined.\n");
        return;
    }
    ...
    while (a) {
        if (policy == 2 && a->priority > apps[apps_n].priority)
            break;
        if (policy == 3 && (a->deadline > apps[apps_n].deadline || \
                a->deadline == apps[apps_n].deadline && a->priority > apps[apps_n].priority))
            break;
        pred_a = a;
        a = a->next_app;
	}
    if (pred_a)
        pred_a->next_app = apps + apps_n;
    else
        head_queue = apps + apps_n;
    apps[apps_n].next_app = a;
   ...
   while (head_queue) {
        ...
        while (running_app) {
            ...
            if (policy == 2 && running_app->next_app && running_app->next_app->priority > running_app->priority)
                break;
            if (policy == 3 && running_app->next_app && \
                    (running_app->next_app->deadline > running_app->deadline || \
                    (running_app->next_app->deadline == running_app->deadline && \
                    running_app->next_app->priority > running_app->priority)))
                break;
        ...
        if (!running_app || running_app->time_continue > time) {
